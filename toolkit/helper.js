...
const getWelTxt = id => getGc(db, id)?.gbFilter?.Welcome?.welcomeText?.trim() || "✨ member baru yaa wak @user di grup!";
const enLeft = id => getGc(db, id)?.gbFilter?.Left?.gcLeft === true;
const getLeftTxt = id => getGc(db, id)?.gbFilter?.Left?.leftText?.trim() || "🐾 out aja sono hama kontol najis babi @user!";
      ...
      await conn.sendMessage(chatId, {
        ...
        contextInfo: {
          externalAdReply: {
            ...
            thumbnailUrl: "https://files.catbox.moe/hfshri.jpg",
            mediaType: 1,
